...
LIBTCCAPI TCCState *tcc_new(void)
{
    ...
    /* standard defines */
    #ifdef _WIN32
        tcc_define_symbol(s, "__OBJ_TYPE__", ".exe");
    #elif defined(__linux__) || defined(__unix__)
        tcc_define_symbol(s, "__OBJ_TYPE__", "elf");
    #endif
...
LIBTCCAPI void tcc_execmain(TCCState* s, const char* name)
{
    void(*sym)() = tcc_get_symbol(s, name);
    if(!sym)
    {
        printf("No symbol named %s was found", name);
        return;
    }
    sym();
}
